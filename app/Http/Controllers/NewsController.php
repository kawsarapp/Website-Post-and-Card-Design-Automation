...
use App\Services\TelegramService; // ✅ টেলিগ্রাম সার্ভিস
...
class NewsController extends Controller
{
    ...
    public function toggleAutomation(Request $request)
    {
        ...
        $currentState = Cache::get('auto_post_enabled', false);
        $newState = !$currentState;
        
        Cache::put('auto_post_enabled', $newState);
        
        if ($request->has('interval') && $request->interval > 0) {
            Cache::put('auto_post_interval', $request->interval);
        }

        if ($newState) {
            Cache::put('next_auto_post_time', now());
        } else {
            Cache::forget('next_auto_post_time');
        }

        $interval = Cache::get('auto_post_interval', 5);
        $status = $newState ? "চালু (প্রতি {$interval} মি. পর পর)" : 'বন্ধ';

        return back()->with('success', "অটোমেশন {$status} করা হয়েছে।");
    }

    // ম্যানুয়াল বা অটোমেটিক পোস্ট
    public function postToWordPress($id)
    {
        ...
        try {
            ...
            if ($wpPost) {
                ...
                // ✅ টেলিগ্রামে পাঠানো (সঠিক জায়গায় বসানো হয়েছে)
                $postLink = $wpPost['link'] ?? $news->original_link; 
                $this->telegram->send($finalTitle, $postLink);

                return back()->with('success', "পোস্ট পাবলিশ হয়েছে এবং টেলিগ্রামে পাঠানো হয়েছে! ID: " . $wpPost['id']);
            } else {
